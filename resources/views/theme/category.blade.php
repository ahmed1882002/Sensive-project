                        ...
                        @foreach ($blogs as $blogPost)
                            <div class="col-md-6">
                                <div class="single-recent-blog-post card-view">
                                    <div class="thumb">
                                        <img class="img-fluid" src="{{ asset('storage/blogs/' . $blogPost->image) }}"
                                            alt="" style="width: 100%;">
                                        <ul class="thumb-info">
                                            <li><a href="#"><i
                                                        class="ti-user"></i>{{ $blogPost->user->name ?? 'Admin' }}</a></li>
                                            <li><a href="#"><i
                                                        class="ti-themify-favicon"></i>{{ $blogPost->comments()->count() }}
                                                    Comments</a></li>
                                        </ul>
                                    </div>
                                    <div class="details mt-20">
                                        <a href="{{ route('blogs.show', ['blog' => $blogPost->id]) }}">
                                            <h3>{{ $blogPost->name }}</h3>
                                        </a>
                                        <p>{{ Str::limit($blogPost->description, 100) }}</p>
                                        <a class="button" href="{{ route('blogs.show', ['blog' => $blogPost->id]) }}">Read
